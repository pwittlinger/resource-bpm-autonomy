...
def get_instantiated_data(file_path):
    ...
    templates = {
        "add_action": {
            "params": ["?a", "?r"],
            "pre": "(and (not (after_add)) (not (complete_sync ?a)) (not (after_sync)) (not (failure)) (not (after_add_check)) (recovery_finished) (can_work ?a ?r) (not (exists (?c - constraint) (violated ?c) )) (exists (?s1 - automaton_state ?s2 - automaton_state) (and (cur_s_state ?s1) (not (goal_state ?s1)) (not (failure_state ?s1)) (automaton ?s1 ?a ?s2) (not (failure_state ?s2)) ) ) )",
        ...
        "check_added_parameter_model": {
            ...
            "eff": "(and (after_add_check) (not (after_add)) (forall (?pn - parameter_name ?s1 - automaton_state ?s2 - automaton_state) (when (and (cur_s_state ?s1) (automaton ?s1 ?a ?s2) (has_constraint ?a ?pn ?s1 ?s2) (or (not (has_added_parameter_aut ?a ?pn ?s1)) (and (has_added_parameter_aut ?a ?pn ?s1) (or (< (added_parameter_aut ?a ?pn ?s1) (majority_constraint ?a ?pn ?s1 ?s2)) (> (added_parameter_aut ?a ?pn ?s1) (minority_constraint ?a ?pn ?s1 ?s2)) (< (added_parameter_aut ?a ?pn ?s1) (interval_constraint_lower ?a ?pn ?s1 ?s2)) (> (added_parameter_aut ?a ?pn ?s1) (interval_constraint_higher ?a ?pn ?s1 ?s2)) (< (added_parameter_aut ?a ?pn ?s1) (equality_constraint ?a ?pn ?s1 ?s2)) (> (added_parameter_aut ?a ?pn ?s1) (equality_constraint ?a ?pn ?s1 ?s2)) (= (added_parameter_aut ?a ?pn ?s1) (inequality_constraint ?a ?pn ?s1 ?s2)) ) ) ) ) (invalid ?s1 ?a ?s2)) ) )"
        },
        "add_move_automata": {
            "params": ["?a"],
            "pre": "(and (complete_sync ?a) (not (after_add)) (after_add_check) (exists (?s1 - automaton_state ?s2 - automaton_state) (and (not (goal_state ?s1)) (not (invalid ?s1 ?a ?s2)) (cur_s_state ?s1) (automaton ?s1 ?a ?s2) (not (failure_state ?s2)) ) ) )",
            "eff": "(and (increase (total_cost) 0) (not (after_add_check)) (forall (?s1 - automaton_state ?s2 - automaton_state) (when (and (automaton ?s1 ?a ?s2) (cur_s_state ?s1) (not (failure_state ?s2)) (not (invalid ?s1 ?a ?s2)) ) (and (not (cur_s_state ?s1)) (cur_s_state ?s2) ) ) ) (forall (?s1 - automaton_state ?s2 - automaton_state ?c - constraint) (when (and (not (invalid ?s1 ?a ?s2)) (automaton ?s1 ?a ?s2) (cur_s_state ?s1) (failure_state ?s2) (associated ?s1 ?c) (associated ?s2 ?c) ) (and (not (cur_s_state ?s1)) (cur_s_state ?s2) (failure) (violated ?c) )) ) (forall (?s1 - automaton_state ?pn - parameter_name) (when (has_added_parameter_aut ?a ?pn ?s1) (not (has_added_parameter_aut ?a ?pn ?s1))) ) (forall (?s1 - automaton_state ?s2 - automaton_state) (when (and (automaton ?s1 ?a ?s2) (invalid ?s1 ?a ?s2) ) (not (invalid ?s1 ?a ?s2)) ) ) (not (complete_sync ?a)) ) "
        },
        "validate-payload": {
            "params": ["?t1", "?a", "?t2"],
            "pre": "(and (cur_t_state ?t1) (trace ?t1 ?a ?t2) (not (after_sync)) (not (after_add)) (not (failure)) (not (complete_sync ?a)) (not (after_add_check)) (not (exists (?c - constraint) (violated ?c) )) (not (recovery_finished)) (exists (?s1 - automaton_state ?s2 - automaton_state ) (and (cur_s_state ?s1) (not (failure_state ?s1)) (not (goal_state ?s1)) (automaton ?s1 ?a ?s2) (not (invalid ?s1 ?a ?s2)) ) ))",
            "eff": "(and (increase (total_cost) 0) (after_sync) (forall (?pn - parameter_name ?s1 - automaton_state ?s2 - automaton_state) (when (and (cur_s_state ?s1) (automaton ?s1 ?a ?s2) (has_constraint ?a ?pn ?s1 ?s2) (or (not (has_parameter ?a ?pn ?t1 ?t2)) (< (trace_parameter ?a ?pn ?t1 ?t2) (majority_constraint ?a ?pn ?s1 ?s2)) (> (trace_parameter ?a ?pn ?t1 ?t2) (minority_constraint ?a ?pn ?s1 ?s2)) (< (trace_parameter ?a ?pn ?t1 ?t2) (interval_constraint_lower ?a ?pn ?s1 ?s2)) (> (trace_parameter ?a ?pn ?t1 ?t2) (interval_constraint_higher ?a ?pn ?s1 ?s2)) (< (trace_parameter ?a ?pn ?t1 ?t2) (equality_constraint ?a ?pn ?s1 ?s2)) (> (trace_parameter ?a ?pn ?t1 ?t2) (equality_constraint ?a ?pn ?s1 ?s2)) (= (trace_parameter ?a ?pn ?t1 ?t2) (inequality_constraint ?a ?pn ?s1 ?s2)) ) ) (invalid ?s1 ?a ?s2) ) ) )"
        },
        "sync-actions": {
            "params": ["?t1", "?t2", "?a"],
            "pre": "(and (not (after_add)) (not (failure)) (not (after_add_check)) (not (complete_sync ?a)) (after_sync) (cur_t_state ?t1) (trace ?t1 ?a ?t2) (exists (?s1 - automaton_state ?s2 - automaton_state) (and (cur_s_state ?s1) (automaton ?s1 ?a ?s2) ) ) )",
            "eff": "(and (increase (total_cost) 0) (not (cur_t_state ?t1)) (cur_t_state ?t2) (when (final_t_state ?t2) (recovery_finished)) (not (after_sync)) (forall (?s1 - automaton_state ?s2 - automaton_state) (when (and (not (invalid ?s1 ?a ?s2)) (automaton ?s1 ?a ?s2) (cur_s_state ?s1) (not (failure_state ?s2)) ) (and (not (cur_s_state ?s1)) (cur_s_state ?s2) )) ) (forall (?s1 - automaton_state ?s2 - automaton_state ?c - constraint) (when (and (not (invalid ?s1 ?a ?s2)) (automaton ?s1 ?a ?s2) (cur_s_state ?s1) (failure_state ?s2) (associated ?s1 ?c) (associated ?s2 ?c) ) (and (not (cur_s_state ?s1)) (cur_s_state ?s2) (failure) (violated ?c) )) ) (forall (?s1 - automaton_state ?s2 - automaton_state) (when (and (invalid ?s1 ?a ?s2) (automaton ?s1 ?a ?s2) ) (not (invalid ?s1 ?a ?s2)) ) ) )"
        },
        "reset": {
            "params": ["?c"],
            "pre": "(and (violated ?c) (not (after_sync)) (not (after_add)) (not (after_add_check)))",
            "eff": "(and (increase (total_cost) (violation_cost ?c)) (forall (?s1 ?s2 - automaton_state) (when (and (cur_s_state ?s1) (not (goal_state ?s1))  (initial_state ?s2) (associated ?s1 ?c) (associated ?s2 ?c) (not (failure_state ?s2)) ) (and (not (cur_s_state ?s1)) (cur_s_state ?s2) ) ) ) (not (failure)) (not (violated ?c)) )"
        },
        "skip-unused": {
            "params": ["?t1", "?a", "?t2"],
            "pre": "(and (cur_t_state ?t1) (trace ?t1 ?a ?t2) (not (recovery_finished)) (not (exists (?s1 - automaton_state ?s2 - automaton_state) (and (automaton ?s1 ?a ?s2) (not (failure_state ?s2)) ) )) )",
            "eff": "(and (not (cur_t_state ?t1)) (cur_t_state ?t2) (when (final_t_state ?t2) (recovery_finished)))"
        },
        "validate-failure": {
            "params": ["?c"],
            "pre": "(and (not (after_sync)) (not (after_add)) (not (after_add_check)) (exists (?s1 - automaton_state) (and (cur_s_state ?s1) (associated ?s1 ?c) (not (goal_state ?s1))  ) ) )",
            "eff": "(and (forall (?s1 - automaton_state) (when (and (cur_s_state ?s1) (associated ?s1 ?c) (not (goal_state ?s1)) ) (violated ?c)) ) )"
        }
